...
datasets = ["0.05", "0.25", "0.50", "0.75", "1.00"] # these are the q values
seeds = ["0", "1", "2", "3", "4"]
...
for dataset in datasets:  
    ...
    for class_weights in class_weights_list:
        ...
        for seed in seeds: 
            teacher_val = np.load(f'saved_arrays/auc_ovr_diff_teacher_sbm{dataset}_{seed}_c={class_weights}_q.npy')
            student_val = np.load(f'saved_arrays/auc_ovr_diff_student_sbm{dataset}_{seed}_c={class_weights}_q.npy')
            teacher_auc = np.load(f'saved_arrays/auc_ovr_overall_teacher_sbm{dataset}_{seed}_c={class_weights}_q.npy')
            student_auc = np.load(f'saved_arrays/auc_ovr_overall_student_sbm{dataset}_{seed}_c={class_weights}_q.npy')
            teacher_acc = np.load(f'saved_arrays/acc_teacher_sbm{dataset}_{seed}_c={class_weights}_q.npy')
            student_acc = np.load(f'saved_arrays/acc_student_sbm{dataset}_{seed}_c={class_weights}_q.npy')
...
for class_weights in class_weights_list:
    ...
    plt.title('Group 0 - 1 AUC Diff between Teacher and Student vs Ratio between Intra/Inter Edge Probability')
    plt.xlabel('Ratio between intra/inter edge probability')
    plt.ylabel('Difference between Teacher and Student')
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig(f'figs_final/auc_bar_diff_c={class_weights}_q.png')
    plt.close() 
    ...
    ax.set_title('Group 0 - 1 AUC Comparison vs Ratio between Intra/Inter Edge Probability')
    ax.set_xlabel('Ratio between intra/inter edge probability')
    ax.set_ylabel('Group 0 - 1 AUC')
    ax.set_xticks([i + bar_width/2 for i in index])
    ax.set_xticklabels(categories, rotation=45)
    ax.legend()
    plt.tight_layout()
    plt.savefig(f'figs_final/auc_bar_comp_c={class_weights}_q.png')
    plt.close() 
    ...
    plt.figure(figsize=(10, 6))
    plt.bar(categories, avg_diffs, yerr=std_diffs/np.sqrt(len(seeds)), capsize=5)
    plt.title('Overall AUC Diff (Teacher-Student) vs Ratio between Intra/Inter Edge Probability')
    plt.xlabel('Ratio between intra/inter edge probability')
    plt.ylabel('Overall AUC Diff (Teacher-Student)')
    plt.xticks(rotation=45)

    plt.tight_layout()
    plt.savefig(f'figs_final/auc_c={class_weights}_q.png')
    plt.close() 
    ...
    plt.figure(figsize=(10, 6))
    plt.bar(categories, avg_diffs, yerr=std_diffs/np.sqrt(len(seeds)), capsize=5)
    plt.title('Overall Accuracy Diff (Teacher-Student) vs Ratio between Intra/Inter Edge Probability')
    plt.xlabel('Ratio between intra/inter edge probability')
    plt.ylabel('Overall Accuracy Diff (Teacher-Student)')
    plt.xticks(rotation=45)

    plt.tight_layout()
    plt.savefig(f'figs_final/acc_c={class_weights}_q.png')
    plt.close()
